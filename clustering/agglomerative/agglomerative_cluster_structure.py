...
class AWardClusterStructure(AgglomerativeClusterStructure):
    class Cluster(ClusterStructure.Cluster):
        ...
        def __init__(self, label, cluster_structure):
            super().__init__(label, cluster_structure)
            self._w = None

        def _update(self):
            """Updates cluster centroid and w parameter"""
            cluster_points = self._cluster_structure.data[self._points_indices]
        ...
        def w(self):
            if self._w is None:
                raise AWardClusterStructure.Cluster.WUndefinedException(
                    "w is undefined for an empty cluster: {}".format(self))
            return self._w
    ...
    def _make_new_cluster(self, label):
        return AWardClusterStructure.Cluster(label, self)

    def merge(self, cluster1_label, cluster2_label):
        """Merges two clusters to one.
        :param AWardCluster cluster1_label: a label of the first cluster to merge
        :param AWardCluster cluster2_label: a label of the second cluster to merge"""
...
class AWardPBClusterStructure(AgglomerativeClusterStructure):
    class Cluster(ClusterStructure.Cluster):
        ...
        def __init__(self, label, cluster_structure, weights=None):
            ...
            super().__init__(label, cluster_structure)
            if weights is None:
                self._weights = np.full(shape=(1, self.cluster_structure.dim_cols),
                                        fill_value=1 / self.cluster_structure.dim_cols)
            else:
    ...
    def _make_new_cluster(self, label):
        return AWardPBClusterStructure.Cluster(label, self)
...
class AWardPBClusterStructureMatlabCompatible(AWardPBClusterStructure):
    ...
    def _make_new_cluster(self, label):
        return AWardPBClusterStructureMatlabCompatible.Cluster(label, self)
