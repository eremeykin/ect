...
class AWardClusterStructure(AgglomerativeClusterStructure):
    ...
    class Cluster(ClusterStructure.Cluster):
        ...
        def __init__(self, cluster_structure, points_indices, centroid=None, w=None):
            ...
            if centroid is None:
                centroid = np.mean(self._cluster_points, axis=0)  # centroid update to the component-wise mean
            self._centroid = centroid
            # Special property of cluster calculated as sum of square euclidean distance from each point to centroid
            if w is None:
                w = np.sum((self._cluster_points - self.centroid) ** 2)
            self._w = w
    ...
    def merge(self, cluster1, cluster2):
        ...
        new_cluster = AWardClusterStructure.Cluster(self, new_points_indices,
                                                    centroid=new_centroid, w=new_w)
